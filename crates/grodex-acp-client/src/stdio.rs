...
impl StdioClient {
    pub fn spawn_agent_subprocess(cmd: &str, args: &[&str]) -> Result<Self> {
        Self::spawn_agent_subprocess_in(cmd, args, std::env::current_dir().unwrap_or_default())
    }

    /// Spawn the agent subprocess with its working directory set to `cwd`.
    /// `grodex serve` resolves the workspace + config from its own cwd, so
    /// a frontend that opens different projects must set this per spawn.
    pub fn spawn_agent_subprocess_in(cmd: &str, args: &[&str], cwd: impl AsRef<Path>) -> Result<Self> {
