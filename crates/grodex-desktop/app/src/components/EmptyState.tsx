...
export const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  const quickTags = [
    { label: '应用开发', icon: <Smartphone className="w-3.5 h-3.5 text-secondary" />, prompt: '为当前工程构建一个现代化的全栈任务管理模块' },
    { label: '项目理解', icon: <FileText className="w-3.5 h-3.5 text-secondary" />, prompt: '解析当前 grodex 项目架构、核心模块依赖与入口流程' },
    { label: '游戏创意', icon: <Dices className="w-3.5 h-3.5 text-secondary" />, prompt: '基于 HTML5 Canvas 和 TypeScript 构建一个流畅的太空战机小游戏' },
    { label: '工具脚本', icon: <FileCode2 className="w-3.5 h-3.5 text-secondary" />, prompt: '编写自动化打包构建与 Rust/TypeScript 质量检查测试脚本' },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold text-primary tracking-tight font-sans">
          Code with Grodex
        </h1>
      </div>
      ...
      <div className="w-full max-w-2xl rounded-2xl border border-hairline bg-white shadow-sm p-4 transition-all focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/10">
        ...
        <div className="flex items-center justify-between pt-2 mt-1 border-t border-hairline-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <div
              className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-well border border-hairline text-[11px] text-accent font-medium"
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              className="p-1.5 rounded-md hover:bg-well text-secondary transition-colors"
              title="语音输入"
            >
            ...
            <button
              ...
              onClick={handleSend}
              className="w-8 h-8 rounded-full bg-accent hover:bg-accent-hover text-white flex items-center justify-center transition-all active:scale-95 shadow-sm"
              title="发送指令"
            >
      ...
      <div className="flex items-center justify-center gap-2.5 mt-8 flex-wrap">
        {quickTags.map((tag, idx) => (
          ...
          >
            {tag.icon}
            <span className="font-medium group-hover:text-primary">{tag.label}</span>
          </button>
        ))}
      </div>
    </div>
