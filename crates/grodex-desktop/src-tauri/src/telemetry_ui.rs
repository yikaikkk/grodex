...
#[tauri::command]
pub async fn telemetry_overview() -> Result<TelemetryOverview, String> {
    tauri::async_runtime::spawn_blocking(move || -> Result<TelemetryOverview, String> {
        let conn = open_db()?;
    ...
    .await
    .map_err(|e| format!("telemetry task join error: {e}"))?
}
...
#[tauri::command]
pub async fn telemetry_doctor() -> Result<DoctorDto, String> {
    tauri::async_runtime::spawn_blocking(move || -> Result<DoctorDto, String> {
        let conn = open_db()?;
